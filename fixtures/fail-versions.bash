# SUCCESS
# *INFO* Generating Bash script for testing for GHC versions: 8.0.1 8.0.2 8.2.1 8.2.2 8.4.1 8.4.2 8.4.3 8.4.4 8.6.1 8.6.2 8.6.3 8.6.4 8.6.5 8.8.1 8.8.2 8.8.3 8.8.4 8.10.1 8.10.2 8.10.3 8.10.4 8.10.5 8.10.6 8.10.7
#!/bin/bash
# shellcheck disable=SC2086,SC2016,SC2046
# REGENDATA ["bash","fail-versions.project"]
...
CFG_JOBS="8.10.7 8.10.6 8.10.5 8.10.4 8.10.3 8.10.2 8.10.1 8.8.4 8.8.3 8.8.2 8.8.1 8.6.5 8.6.4 8.6.3 8.6.2 8.6.1 8.4.4 8.4.3 8.4.2 8.4.1 8.2.2 8.2.1 8.0.2 8.0.1"
...
echo_to cabal.project "packages: $SRCDIR/servant-client-core"
echo_to cabal.project "packages: $SRCDIR/servant-docs"
echo_to cabal.project "packages: $SRCDIR/servant-foreign"
...
PKGDIR_servant_client_core="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/servant-client-core-[0-9.]*')"
PKGDIR_servant_docs="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/servant-docs-[0-9.]*')"
PKGDIR_servant_foreign="$(find "$BUILDDIR/unpacked" -maxdepth 1 -type d -regex '.*/servant-foreign-[0-9.]*')"
...
echo_to cabal.project "packages: ${PKGDIR_servant_client_core}"
echo_to cabal.project "packages: ${PKGDIR_servant_docs}"
echo_to cabal.project "packages: ${PKGDIR_servant_foreign}"
...
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package servant-client-core"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package servant-docs"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package servant-foreign"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "package servant-server"
echo_if_to $((HCNUMVER >= 80200)) cabal.project "    ghc-options: -Werror=missing-methods"
cat >> cabal.project <<EOF
EOF
$HCPKG list --simple-output --names-only | perl -ne 'for (split /\s+/) { print "constraints: $_ installed\n" unless /^(servant|servant-client|servant-client-core|servant-docs|servant-foreign|servant-server)$/; }' >> cabal.project.local
run_cmd cat cabal.project
...
run_cmd ${CABAL} -vnormal check
change_dir "${PKGDIR_servant_client_core}"
run_cmd ${CABAL} -vnormal check
change_dir "${PKGDIR_servant_docs}"
run_cmd ${CABAL} -vnormal check
change_dir "${PKGDIR_servant_foreign}"
run_cmd ${CABAL} -vnormal check
